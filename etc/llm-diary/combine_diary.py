...
def create_group_data_wrapper(
    ...
    """
    Combines individual diary pickle files for a specified group into a single
    pickle file and generates a visualization of the combined diary percentages.

    The individual diary files are expected to be named like:
    "diary_{people_type}_{age_group}_{day_type}.p"

    Args:
        workdir (str): The directory containing the individual diary pickle files
                       and where the combined output will be saved.
        group_name (str): The name for this combined group (e.g., "student_weekday").
        people_list (list): List of people types (e.g., ["student", "worker1"]).
        age_list (list): List of age groups (e.g., ["6-18", "19-25"]).
        day_list (list): List of day types (e.g., ["weekday", "weekend"]).
    """
    all_data = []
    filename = ""
...
def create_all_data(workdir: str):
    """
    Combines all group-level combined diary pickle files (generated by
    `create_group_data_wrapper`) in the specified `workdir`/combined directory
    into a single master pickle file named "llm_diary.pickle".

    This master file contains two main keys:
    - "percentage": A dictionary where keys are group names and values are
                    DataFrames of hourly location percentages for that group.
    - "data": A dictionary where keys are group names and values are
              DataFrames of raw diary entries (Hour, Location, People_id)
              for that group.

    Args:
        workdir (str): The base working directory. This function expects
                       the group-combined pickle files to be in a subdirectory
                       named "combined" within this workdir.
    """
